...
def rebuild_test_frame() -> pd.DataFrame:
    ...
    train.py used (group-aware split by config.GROUP_SPLIT_COLS, see
    splits.py), so sampled rows were never trained or tuned on, and no
    point's rows straddle train and test."""
    full_dataset = TactileSensorDataset()
    _, _, test_idx = get_group_split_indices(full_dataset.frame)
    return full_dataset.frame.iloc[test_idx]
