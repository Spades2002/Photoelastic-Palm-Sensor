...
def rebuild_test_split(full_dataset: TactileSensorDataset):
    """Recreate the exact same train/val/test split used in train.py (see
    splits.py): a group-aware split by config.GROUP_SPLIT_COLS, so no
    point's rows straddle train and test. This guarantees the samples
    checked here come only from data the model never trained or was tuned
    on, at the point level, not just the row level."""
