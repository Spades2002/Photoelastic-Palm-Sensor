...
# Columns defining a "point" for the grouped train/val/test split (see
# splits.py). Every row sharing the same combination of these columns, e.g.
# all depths, phases, and frame indices captured during one indentation
# cycle at one grid location, is guaranteed to land in the same split, so
# no near-duplicate frames leak across train/val/test.
# Change to [COL_ROW, COL_COL] instead for a stricter split that also holds
# out entire physical grid locations (fewer distinct locations available
# for training, but tests generalisation to genuinely unseen positions).
GROUP_SPLIT_COLS = [COL_ROW, COL_COL, COL_CYCLE]
